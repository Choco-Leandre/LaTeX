...
\includegraphics[scale=0.57]{Bras_robotique.png}
\end{center}
\vfill
\begin{center}
...
{\footnotesize Une IA a été utilisée pour corriger l'orthographe.}\\
...
\noindent{\Large\textbf{Cinématique}}
\phantomsection
\addcontentsline{toc}{subsection}{Généralités}\vspace*{4pt}
...
L'axe de rotation doit être vers nous.
\end{definition}
\columnbreak
...
\end{multicols}
\phantomsection
\addcontentsline{toc}{subsection}{Loi d'entrée-sortie}
...
\pagebreak
\phantomsection
\addcontentsline{toc}{subsection}{Torseur cinématique}
...
= \begin{Bmatrix}
 \omega_x \cdot \vec{x} &+& \omega_y \cdot \vec{y} &+& \omega_z \cdot \vec{z} \\
 v_x \cdot \vec{x} &+& v_y \cdot \vec{y} &+& v_z \cdot \vec{z}  \\
...
\end{definition}
\begin{multicols}{2}
\noindent \begin{propriété}[label=formuledebour]{Formule de Bour}
...
\end{méthode}
\end{multicols}
\begin{exemple}{Dérivée d'un vecteur}
On donne la vitesse au point $A$ du solide 1 par rapport à 0 tel que: $\overrightarrow{v_{A\in 1/0}}= r\dot{\theta}\vec{y_1}$ (avec $r$ constante, et $\left(\frac{\mathrm{d}\vec{y_1}}{\mathrm{d}t}\right)_{\mathcal{R}_0} = -\dot{\theta}\vec{x_1}$).

\noindent \textbf{- Déterminer l'accélération au point A du solide 1 par rapport à 0}
\begin{align*}
\overrightarrow{\gamma_{A\in 1/0}} &= \left(\frac{\mathrm{d}\overrightarrow{v_{A\in 1/0}}}{\mathrm{d}t}\right)_{\mathcal{R}_0} = \frac{\mathrm{d}}{\mathrm{d}t}\left[ r\dot{\theta}\vec{y_1} \right]_{\mathcal{R}_0} = r\ddot{\theta}\vec{y_1} + r\dot{\theta}\left(\frac{\mathrm{d}\vec{y_1}}{\mathrm{d}t}\right)_{\mathcal{R}_0} = r\ddot{\theta}\vec{y_1} + r\dot{\theta}(-\dot{\theta}\vec{x_1}) = \boxed{-r\dot{\theta}^2\vec{x_1} + r\ddot{\theta}\vec{y_1}}
\end{align*}
\end{exemple}
...
\end{center}
\textbf{- Déterminer la loi d'entrée-sortie géométrique:}
...
\end{center}
\end{multicols}
\textbf{- Déterminer la loi d'entrée-sortie (E/S) en vitesse:}\\
\underline{Ce qu'on peut faire:}
\begin{itemize}
\item Faire une fermeture cinématique
\item Dériver notre fermeture géométrique \quad (Ce qui est + simple)
\end{itemize}
Donc: $$\dot{\lambda}(t) = \frac{\mathrm{d}}{\mathrm{d}t} \left[ R - e \sin(\theta(t)) \right] = - e \cdot \frac{\mathrm{d}}{\mathrm{d}t}\left[ \sin(\theta(t)) \right] \xeq{(f\circ g)'} \boxed{- e \cdot \dot{\theta}(t) \cos(\theta(t))=\dot{\lambda}}$$

\textbf{- Déterminer la condition de roulement sans glissement (RSG) entre la came 1 et la soupape 2:}\vspace{-10pt}\\
\begin{multicols}{2}
\begin{align*}
...
\end{align*}
\begin{align*}
\text{{Or,}} \quad \overrightarrow{v_{I\in1/0}} &\xeq{\textrm{Varignon}} \cancel{\overrightarrow{v_{O\in1/0}}} + \overrightarrow{IO} \wedge \overrightarrow{\Omega_{1/0}} \\
...
\end{align*}
\noindent D'où la vitesse de glissement:
\begin{align*}
\overrightarrow{v_{I\in1/2}} &= R\dot{\theta}\overrightarrow{x_0} + e\dot{\theta}\overrightarrow{y_1} + \dot{\lambda}\overrightarrow{y_0} \\
&= (R\dot{\theta} - e\dot{\theta}\sin\theta)\overrightarrow{x_0} + \underbrace{(\dot{\lambda} + e\dot{\theta}\cos\theta)}_{=0\phantom{k}(\textrm{loi d'E/S en }v)}\overrightarrow{y_0}
\end{align*}
\noindent Or, il y a RSG en $I$ tel que: $\overrightarrow{v_{I\in1/2}} = \overrightarrow{0}$, d'où:
$$\overrightarrow{v_{I\in1/2}} \cdot \overrightarrow{x_0} = 0 \iff \boxed{(R - e\sin\theta)\dot{\theta} = 0}$$
\end{multicols}
...
\end{multicols}
\begin{multicols}{2}
\begin{exemple}{Quelques exemples de torseurs \textit{(d'actions mécaniques extérieur)}}
...
\end{multicols}
\pagebreak
\begin{definition}{Tableau des liaisons normalisées pour des efforts statiques}
...
On dit qu'il y a \textbf{mouvement relatif} entre 1 et 0.\\\\\\
...
\end{definition}
\begin{definition}{Durée de vie (formules donnée I think)}
...
\end{definition}
\begin{vocabulaire}{Bague montée serrée ou glissante dans un roulement (Hors Prog.)}
...
Pour former un solide, il est possible de le \textbf{décomposer en plusieurs sous-solide} afin d'en définir plus facilement sa matrice d'inertie. Ainsi, les moments et produits d'inertie vont s'ajouter/se soustraire entre eux à condition d'être au même point.\vspace*{-15pt}
\begin{center}
\includegraphics[scale=0.47]{schematics/Additivité_matrice_d_inertie.pdf} 
\end{center}
\end{propriété}
...
Si une pièce possède \textbf{un axe de révolution}, les \textbf{2 moments d'inertie ne suivant pas l'axe} sont \textbf{égaux} et les produits d'inertie sont nuls.
\end{propriété}
\begin{exemple}{Symétries pour différents cas (Jules a été volontaire)}
On suppose que Jules ne possède qu'un plan de symétrie selon $(O,\vec{x},\vec{y})$.\vspace*{-5pt}
\begin{center}
\includegraphics[scale=0.63]{schematics/Exemples_symétries.pdf} \vspace*{-5pt}
\end{center}
\end{exemple}
...
\textbf{ATTENTION} $d$ est la distance entre les axes et on sera tjr au point $G$.\columnbreak
\begin{center}
...
\begin{definition}{Dynamique}
La \textbf{dynamique} est l'étude des \textbf{efforts} et des \textbf{mouvements} d'un système matériel en tenant compte de sa masse et de son inertie.
\end{definition}
...
\end{Bmatrix}_A$}
\textbf{N.B.:} Pour, $\overrightarrow{\Omega_{1/0}}=\omega\cdot \vec{z}$ et pour une inertie $J$ autour de $(A,\vec{z})$ on aura: $\overrightarrow{\sigma_{A\in 1/0}}\cdot \vec{z}=J\omega$
\end{definition}
...
\end{Bmatrix}_A$}
On pensera à se référer à la \hyperref[formuledebour]{formule de Bour}.
\end{definition}
\begin{definition}{Principe Fondamental de la Dynamique (PFD)}
Pour un ensemble de $n$ solides, dans un référentiel galiléen $\mathcal{R}_0$.
...
\begin{center}
\includegraphics[scale=0.45]{schematics/PFD.pdf} \vspace*{-3pt}
\end{center}
\noindent Ainsi, le PFD s'exprimera: \vspace*{-12pt}
...
On donne:
\begin{center}
\includegraphics[scale=0.5]{schematics/Graphe_d'analyse_Mandrin.pdf}\vspace*{5pt}
\end{center}
