...
Soit la fonction de transfert: $H(p) = \frac{\frac{3}{p} + 6}{2p + \frac{8}{p}}$, mettons là sous forme canonique.
\begin{align*} H(p) = \frac{\frac{3}{p} + 6}{2p + \frac{8}{p}} = \frac{\left(\frac{3}{p} + 6\right) \cdot p}{\left(2p + \frac{8}{p}\right) \cdot p} = \frac{3 + 6p}{8 + 2p^2} \xeq{\text{Factorisation par 3 et 8}} \frac{3(1 + 2p)}{8\left(1 + \frac{1}{4}p^2\right)} = \frac{3}{8} \cdot \frac{1 + 2p}{1 + \frac{1}{4}p^2}
\end{align*}
Avec cette version, on pourra en déduire facilement plusieurs valeurs.
\end{exemple}
\phantomsection
...
Lorsqu'on a 2 entrées (souvent en cas de perturbations), on \textbf{divise notre schéma bloc en deux parties}.\\
Ainsi:
\begin{center}
...
On appelle \textbf{pôle} d'un système, une valeur de la variable de Laplace $p$ pour laquelle la fonction de transfert $H(p)$ tend vers l'infini. Plus simplement, ce sont \textbf{les racines du dénominateur} d'une fonction de transfert.
\end{definition}
...
- On a les pôles.\\
\textbf{Remarques:}\\
Si \textbf{tous les} pôles de la $FTBF$ sont à partie réelle strictement négative, alors le système sera dit \textbf{stable}. Au contraire, si au moins \textbf{un} pôle a une partie réelle positive ou nulle, le système sera dit \textbf{instable}.
\end{méthode}
\begin{definition}{Classe $\alpha$}
La \textbf{classe} d'un système est le nombre d'intégrateurs à l'origine ($\frac{1}{p}$) présents dans sa $FTBO$.\vspace*{-10pt}
$$FTBO(p) = \frac{K \cdot (\dots)}{p^\alpha \cdot (\dots)}$$
...
On définit la \textbf{précision} sur une réponse temporelle comme l'écart entre la consigne et la sortie en régime établi
...
Afin d'augmenter la précision, en pratique on \textbf{augmente le gain}. Cependant, cela a pour effet de réduire nos marges de phase et \textbf{crée de l'instabilité}. On peut aussi \textbf{augmenter la classe de la FTBO}.
\end{propriété}
\phantomsection
