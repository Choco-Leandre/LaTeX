...
\fancyhead[C]{{\fontfamily{phv}\selectfont $\cdots$ Électricité - Léandre Courtoy $\cdots$}}
...
\end{multicols}
\noindent NB: Une bobine est vu comme un composant qui \textbf{dérive}:\\
...
\textbf{Q1. Faire apparaître sur le schéma les caractéristiques $U_1$, $U_2$, $I_1$ et $I_2$ associés aux résistances $R_1$ et $R_2$:}\\
...
\end{definition}
\noindent Les 3 définitions suivantes sont valables \textbf{pour des signaux sinusoïdaux}
...
- La tension la plus faible correspond à la tension nominale d'un enroulement et la tension maximale que peut supporter celui-ci.\\
...
Un \textbf{modulateur statique} is un dispositif traitant l'énergie électrique c'est-à-dire en \textbf{modulant} (rendre compatibles les fonctions de tension, courant et fréquence) et en \textbf{convertissant} (passer d'un type d'énergie à une autre).
\end{definition}
...
\end{multicols}
\end{definition}
\begin{méthode}{Etudier un état de diode par l'absurde}
...
Donc: $u_r(t)=e(t)$ et $i(t)=\frac{u_r(t)}{R}=e(t)$
\columnbreak\\
\underline{Pour $e(t)<0$}:\\
...
Le principe de l'électronique de puissance est de \textbf{commuter à hautes fréquences} un circuit électronique généralement inductif. (dans celui-ci il y a généralement une \textbf{bobine})
\end{propriété}
\phantomsection
...
On appelle \textbf{hacheur série}, un montage électrique permettant d'obtenir une tension moyenne de sortie inférieure à la tension d'entrée en commutant un interrupteur commandé à haute fréquence.
\begin{center}
...
\end{definition}
\begin{exemple}[label=exemple_hacheur_boost]{Etude d'un hacheur Boost}
...
\end{center}
\textbf{- Etablir les expressions de la tension $u_L(t)$ pour les deux phases de fonctionnement}\\
...
On appelle \textbf{MLI} (Modulation de Largeur d'Impulsion) ou PWM, un type de commande où l'on compare un signal de référence sinusoïdal lent (consigne de vitesse) à un signal triangulaire haute frequency (la \textbf{porteuse}).\\
...
\end{center}
\end{propriété}
\begin{méthode}{Etudier la valeur moyenne de la tension d'un PD3}
...
\begin{multicols}{2}
Un \textbf{onduleur} est un convertisseur statique permettant de convertir une tension \textbf{continue} en tension \textbf{alternative} de fréquence réglable.\\
...
\end{multicols}
\end{propriété}
\noindent L'étude reste la même que les hacheurs. Veuillez vous référer à l'\hyperref[exemple_hacheur_série]{\textbf{\textcolor{mygray}{Exemple \ref*{exemple_hacheur_série}}}} et l'\hyperref[exemple_hacheur_boost]{\textbf{\textcolor{mygray}{Exemple \ref*{exemple_hacheur_boost}}}} pour réussir un exercice dessus.
\pagebreak
\phantomsection
...
C'est une \textbf{machine tournante} et un \textbf{convertisseur électromécanique}, permettant la conversion duelle d'énergie.
\begin{center}
...
\formuler{$C_m(t)=K\cdot\Phi\cdot i(t)$}
Pour un flux à aimant permanent:
\formuler{$C_m(t)=K_c\cdot i(t)$}
...
On fera bien attentions aux unités (qui sont différentes).
...
Une machine synchrone est constituée d'un \textbf{stator} (l'induit) et d'un \textbf{rotor} (inducteur) qui interagissent ensemble par l'interaction de leurs champs magnétiques.
\begin{flushleft}
\textbf{Le stator:}\\
...
Avant tout, on essayera de faire un diagramme de Fresnel à l'échelle pour une potentielle exploitation des données de celui-ci.\\
...
\begin{exemple}{Méthode pour construire un diagramme de Fresnel sur un circuit}
\vspace*{-10pt}\begin{multicols}{2}
...
\end{center}
Avec: - $\varphi$ le déphasage de la tension simple par rapport au courant. $\bm{\cos(\varphi)}$ représente \textbf{le facteur de puissance}.\\
...
Etudions la tension d'alimentation d'un moteur synchrone dans son cas le plus défavorable. C'est-à-dire pour une valeur angulaire mécanique $\omega_m=200 rad.s^{-1}$ et un couple mécanique $C_{em}=11N.m$.\\
...
\But{On note des fois $P_{meca}=P_{em}$ comme dans cet exo}\\
...
Parlons des codeurs qui font partie intégrante des sujets et qui font partie de l'étude de moteurs. Ils sont aussi présents dans les exos d'asservissements...
...
\end{center}
On considérera le sens direct (horaire) lorsque la voie A est en avance de $90^\circ$ sur la voie B. De plus, le capteur de gain $K_c$ possède 10 pistes.\\
